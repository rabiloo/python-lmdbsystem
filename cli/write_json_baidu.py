...
from lmdbsystem.write_adapters.json_baidu import JsonBaiduWriteAdapter
...
def main():
    ...
    if args.key_probability is not None:
        options["key_probability"] = args.key_probability
    if args.values_map:
        values_map = {value.split(":")[0]: value.split(":")[1] for value in args.values_map.split(",")}
        options["values_map"] = values_map

    lmdb_obj = Lmdb(JsonBaiduWriteAdapter(path=args.lmdb_file))
    lmdb_obj.write_dir(
